...
export async function buildTextbutler(options: { outdir?: string } = {}): Promise<{ directory: string; manifest: DistributionManifest }> {
  ...
  const inputs = new Map<string, string>([["package.json", sha256(packageBytes)], ["desktop-foundation/release-manifest.json", sha256(companionManifest)]]);
  for (const input of ["scripts/build-textbutler.ts", "scripts/textbutler-distribution.ts", "scripts/support-runtime-policy.ts", "LICENSE", "docs/support-foundation-notice.md"]) inputs.set(input, sha256(await readFile(join(ROOT, input))));
  const result = await Bun.build({ entrypoints: [join(ROOT, "packages/textbutler/src/runtime-entry.ts")], target: "bun", format: "esm", minify: true, splitting: false,
    define: { "import.meta.url": "__TEXTBUTLER_ARTIFACT_URL" },
  ...
  const notices = `# Textbutler local pilot\n\nThis artifact was built locally with Bun 1.3.14. Digests verify integrity; they are not a signed release or reviewed provider attestation. Claude API and native AI routes remain unavailable in this local distribution. No account or messaging activation occurs during build or installation.\n\nPinned runtime dependencies:\n${Object.entries(PINNED_INPUTS).map(([name, pin]) => `- ${name}: ${pin}`).join("\n")}\n\n## Support foundation\n\n${await readFile(join(ROOT, "docs/support-foundation-notice.md"), "utf8")}\n\n${dependencyNotices.join("\n")}\n## Anthropic SDK bundled qs license\n\n${bundledQsLicense}\n`;
  ...
  const manifest: DistributionManifest = { schemaVersion: 1, product: "textbutler", kind: "local-pilot", version,
    ...
    files: Object.fromEntries([...files].map(([name, bytes]) => [name, { sha256: sha256(bytes), bytes: bytes.length }])) as DistributionManifest["files"], providerAdmission: "unavailable" };
...
if (import.meta.main) {
  ...
  try {
    ...
    process.stdout.write(`${JSON.stringify({ ok: true, directory: result.directory, version: result.manifest.version, providerAdmission: "unavailable", detail: "Self-contained local pilot built. Install it with textbutler:install --from DIRECTORY. AI runtime qualification remains unavailable." })}\n`);
