...
const PRODUCTION_DEPLOYMENTS_QUERY = `query MessageLikeMeProductionDeployments(
  $owner: String!
  $name: String!
  $after: String
) {
...
function canonicalVercelProductionUrl(value, label) {
  ...
  if (!VERCEL_PRODUCTION_URL.test(url)) {
    fail(`${label} is not the canonical Message Like Me Vercel Production URL`);
  }
  return url;
}
...
async function main() {
  ...
  if (command === "wait") {
    const testMode = process.env.MESSAGE_LIKE_ME_PROVIDER_TEST_MODE === "1";
    const maxPolls = testMode
      ? Number(process.env.MESSAGE_LIKE_ME_PROVIDER_MAX_POLLS ?? String(MAX_PROVIDER_POLLS))
      : MAX_PROVIDER_POLLS;
    const pollIntervalMilliseconds = testMode
      ? Number(process.env.MESSAGE_LIKE_ME_PROVIDER_POLL_INTERVAL_MS ?? "0")
