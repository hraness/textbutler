...
function exactClientId(value) {
  if (typeof value !== "string" || !CLIENT_ID.test(value)) {
    fail("MLM_RELEASE_APP_CLIENT_ID is not a canonical GitHub App client ID");
  }
  return value;
}
...
export function parseReleaseAppConfiguration(environment) {
  ...
  if (
    apiUrl.href !== "https://api.github.com/" ||
    apiUrl.username !== "" ||
    apiUrl.password !== ""
  ) {
    fail("GITHUB_API_URL is not the exact GitHub Cloud API origin");
  }
  const privateKey = exactEnvironmentString(environment, "MLM_RELEASE_APP_PRIVATE_KEY");
  if (
    Buffer.byteLength(privateKey, "utf8") > MAX_PRIVATE_KEY_BYTES ||
    /\0/u.test(privateKey)
  ) {
    fail("MLM_RELEASE_APP_PRIVATE_KEY is malformed");
  }
  return Object.freeze({
    apiUrl,
    appId: exactPositiveInteger(
      exactEnvironmentString(environment, "MLM_RELEASE_APP_ID"),
      "MLM_RELEASE_APP_ID",
    ),
    appSlug: exactAppSlug(
      exactEnvironmentString(environment, "MLM_RELEASE_APP_SLUG"),
      "MLM_RELEASE_APP_SLUG",
    ),
    clientId: exactClientId(exactEnvironmentString(environment, "MLM_RELEASE_APP_CLIENT_ID")),
    installationId: exactPositiveInteger(
      exactEnvironmentString(environment, "MLM_RELEASE_APP_INSTALLATION_ID"),
...
export function createReleaseAppJwt(input) {
  ...
  try {
    ...
    fail("MLM_RELEASE_APP_PRIVATE_KEY is not a valid private key");
  }
  if (key.asymmetricKeyType !== "rsa") fail("MLM_RELEASE_APP_PRIVATE_KEY is not an RSA key");
  let signature;
  try {
    signature = sign("RSA-SHA256", Buffer.from(unsigned, "ascii"), key);
  } catch {
    fail("MLM_RELEASE_APP_PRIVATE_KEY could not sign the App JWT");
  }
...
function parseInstallationRepositories(value) {
  ...
  if (
    ...
    fail("release App token repository read is not the exact selected Message Like Me repository set");
  }
...
export function parseReleaseAppTokenResponse(value, serverDate) {
  ...
  const expiresMilliseconds = parseSecondTimestamp(
    response.expires_at,
    "release App token expires_at",
  );
  const lifetime = expiresMilliseconds - responseMilliseconds;
  if (
    lifetime < MIN_TOKEN_LIFETIME_MILLISECONDS ||
    lifetime > MAX_TOKEN_LIFETIME_MILLISECONDS
  ) {
...
export async function withReleaseAppToken(options, operation) {
  ...
  let operationError;
  try {
    const appIdentity = await options.inspect({ apiUrl: configuration.apiUrl, jwt });
    parseReleaseAppIdentity(appIdentity, configuration);
    const installationIdentity = await options.inspectInstallation({
      apiUrl: configuration.apiUrl,
      installationId: configuration.installationId,
      jwt,
    });
    parseReleaseAppInstallation(installationIdentity, configuration);
...
function appRequestHeaders(credential) {
  return Object.freeze({
...
async function mintWithFetch(input) {
  const endpoint = new URL(
    `/app/installations/${String(input.installationId)}/access_tokens`,
    input.apiUrl,
  );
  ...
  if (response.status !== 201) {
    fail(`release App token mint returned HTTP ${String(response.status)}`);
  }
...
async function inspectWithFetch(input) {
  ...
  if (response.status !== 200) {
    fail(`release App identity read returned HTTP ${String(response.status)}`);
  }
...
async function inspectInstallationWithFetch(input) {
  const endpoint = new URL(
    `/app/installations/${String(input.installationId)}`,
    input.apiUrl,
  );
  const response = await fetch(endpoint, {
    headers: appRequestHeaders(`Bearer ${input.jwt}`),
  ...
  if (response.status !== 200) {
    fail(`release App installation read returned HTTP ${String(response.status)}`);
  }
...
async function fetchRevocationDeletionResponse(input, request) {
  ...
  try {
    response = await input.fetchImplementation(request.url, {
      headers: appRequestHeaders(`Bearer ${input.token}`),
      method: "DELETE",
      redirect: "error",
...
export async function revokeReleaseAppTokenWithConvergence(input) {
  ...
  let deletionError;
  try {
    deletion = await fetchRevocationDeletionResponse(shared, { url: deleteUrl });
