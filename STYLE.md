...
- Textbutler's development status is one statement, `SITE_STATUS` in `site/app/_lib/site.ts`. The site renders that constant where it states the status and links to `/#development` elsewhere; the README repeats the same sentences, and a test keeps them equal. Change the status there, not page by page. [`docs/textbutler/readiness.md`](docs/textbutler/readiness.md) holds the detailed readiness table.
- Put each limit beside the feature it limits, in the reader's terms: “Running from source never writes AI replies” beside AI replies, not a composition-admission caveat in every section.
- Name the product Textbutler in prose. `TextButler.app` is the app bundle's file name; keep it in code formatting when you mean the file. Write xcb in lowercase.
- Message Like Me is the retired predecessor. Name it only for the legacy history tools, the `@hraness/message-like-me` package, and “formerly” notes.
