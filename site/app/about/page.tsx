...
export default function AboutPage() {
  return <><SiteHeader /><main className="document-page" id="main-content" tabIndex={-1}>
    ...
    <section className="about-sources" aria-labelledby="about-build-title"><div><h2 id="about-build-title">Start with the source pilot</h2></div><div><p>The guided terminal supports iMessage, WhatsApp and Beeper connections through Ghostget, an inbox and replies you write yourself. The source CLI has no qualified AI reply engine. Claude API requires a reviewed compiled runtime supplied by a trusted integration; Claude Code and Codex remain unavailable. New installations start paused. Live delivery needs verification on your account. The optional native menu retrieves a prebuilt runner; no local Rust build is needed.</p><a href={`${GITHUB_URL}/blob/main/docs/textbutler/getting-started.md`}>Open the guided setup instructions</a><p><a href={ARCHITECTURE_URL}>Read the architecture</a></p></div></section>
