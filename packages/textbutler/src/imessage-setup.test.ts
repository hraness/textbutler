...
function view(revision: number, managed: boolean, permissions: Record<string, string>) {
  return { version: "0.18.18", accountId: ID, accounts: [{ id: ID, provider: "imessage", kind: "linked-device-store", subject: SUBJECT, revision: "c".repeat(64), status: "configured", source: null, tokenStorage: null }],
...
async function processFixture(mode = "normal") {
  ...
  await writeFile(join(packageRoot, "package.json"), JSON.stringify({ name: "@hraness/ghostget", version: "0.18.18", type: "module" }), { mode: 0o600 });
...
test("connector releases without the imsg resource fix are refused before launch", async () => {
  for (const version of ["0.18.16", "0.18.17"]) {
